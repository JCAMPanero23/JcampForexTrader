...
public:
   ...
   //+------------------------------------------------------------------+
   //| Export Signal to JSON File (with components)                     |
   ...
   bool ExportSignalWithComponents(const SignalExportData &data, const StrategySignal &signal)
   {
      ...
      // Build JSON content with components
      string json = BuildJSONWithComponents(data, signal);
   ...
   //+------------------------------------------------------------------+
   //| Export Signal to JSON File (without components)                  |
   //| Creates {SYMBOL}_signals.json (e.g., EURUSD_signals.json)       |
   //+------------------------------------------------------------------+
   bool ExportSignal(const SignalExportData &data)
   {
      ...
      // Build JSON content without components
                                  ...
                                  bool dynamicRegimeTriggered = false)
   {
      ...
      return ExportSignalWithComponents(data, signal);
   }
...
private:
   ...
   //+------------------------------------------------------------------+
   //| Build JSON String (without components)                           |
   //+------------------------------------------------------------------+
   ...
   //+------------------------------------------------------------------+
   //| Build JSON String (with component scores)                        |
   //| ✅ Exports detailed component breakdown for dashboard            |
   //+------------------------------------------------------------------+
   string BuildJSONWithComponents(const SignalExportData &data, const StrategySignal &signal)
   {
      string json = "{\n";
      json += "  \"symbol\": \"" + data.symbol + "\",\n";
      json += "  \"timestamp\": \"" + TimeToString(data.timestamp, TIME_DATE|TIME_MINUTES|TIME_SECONDS) + "\",\n";
      ...
      // ✅ Export component scores for detailed dashboard visualization
      json += "  \"components\": {\n";
